...
# 配置日志
# 设置日志
log_file = Path('logs/gee_download.log')
...
def split_region(geometry: gpd.GeoDataFrame, col_size: float, row_size: float) -> List[Tuple[float, float, float, float]]:
    """将一个区域分割成更小的矩形。"""
...
def download_sentinel2_data(region: ee.Geometry, start_date: str, end_date: str, output_folder: str, cloud_cover_threshold: int = 20):
    """下载给定区域和时间段的Sentinel-2数据。"""
    logger.info(f"下载{start_date}到{end_date}期间的Sentinel-2数据")
    
    os.makedirs(output_folder, exist_ok=True)
    
    rectangles = split_region(region, 0.1, 0.1)  # 假设我们想要 0.1 x 0.1 度的矩形
...
def main(region_path: str, output_folder: str, start_date: str, end_date: str):
    """下载Sentinel-2数据的主函数。"""
    try:
        authenticate_gee()
        ...
        # 下载数据
        download_sentinel2_data(ee_region, start_date, end_date, output_folder)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="从Google Earth Engine下载Sentinel-2数据")
    parser.add_argument("--region_path", type=str, required=True, help="区域shapefile的路径")
    parser.add_argument("--output_folder", type=str, required=True, help="输出文件夹的路径")
    ...
    main(args.region_path, args.output_folder, args.start_date, args.end_date)