...
def load_data(logger, file_path, label_col):
...
def get_raster_info(logger, raster_path):
    logger.info(f"正在获取 {raster_path} 的栅格信息")
    with rasterio.open(raster_path) as src:
        return src.profile, src.shape, src.bounds

def compare_models_and_train_kriging(logger, X, y, rf_model, feature_names, coord_cols, use_rk=False, test_size=0.2, random_state=42):
...
def predict_chunk(logger, model, feature_data, coord_data, is_rfrk, feature_names):
...
def process_raster_chunk(logger, model, feature_files, window, coord_cols, is_rfrk, feature_names, mask=None):
    ...
    if np.sum(valid_pixels) > 0:
        if is_rfrk:
            ...
            predictions = predict_chunk(logger, model, feature_array, coord_array, is_rfrk, feature_names)
        else:
            predictions = predict_chunk(logger, model, feature_array, None, is_rfrk, feature_names)
...
def create_mask_from_shapefile(logger, shapefile_path, raster_path):
    logger.info(f"正在从 {shapefile_path} 创建掩码")
...
def predict_soil_property(logger, model, feature_dir, output_path, coord_cols, is_rfrk, feature_names, shapefile_path=None, chunk_size=1000):
    ...
    mask = None
    if shapefile_path:
        mask = create_mask_from_shapefile(logger, shapefile_path, feature_files[0])
    ...
    with rasterio.open(output_path, 'w', **profile) as dst:
        for row in tqdm(range(0, height, chunk_size), desc="处理行"):
            for col in range(0, width, chunk_size):
                ...
                chunk_result = process_raster_chunk(logger, model, feature_files, window, coord_cols, is_rfrk, feature_names, chunk_mask)
...
def predict_soil_properties(logger, model_dir, feature_dir, output_dir, training_data_path, coord_cols, use_rk=False, shapefile_path=None):
    ...
    for model_file in tqdm(model_files, desc="预测土壤属性"):
        ...
        model, feature_names = load_model(logger, model_file)
        
        training_data = load_data(logger, training_data_path, property_name)
        ...
        updated_model, is_rfrk = compare_models_and_train_kriging(logger, X, y, model, available_features, coord_cols, use_rk)

        output_path = output_dir / f"{property_name}_prediction.tif"
        predict_soil_property(logger, updated_model, feature_dir, output_path, coord_cols, is_rfrk, available_features, shapefile_path)

    logger.info("所有土壤属性预测完成")

def main(log_file,model_dir, feature_dir, output_dir, training_data_path, coord_cols, use_rk, shapefile_path=None):
    # 设置日志
    if log_file:
    ...
    logger.info("开始预测土壤属性")
    try:
        predict_soil_properties(logger, model_dir, feature_dir, output_dir, training_data_path, coord_cols, use_rk, shapefile_path)
        logger.info("土壤属性预测完成")
    except Exception as e:
        logger.error(f"预测土壤属性过程中发生错误: {str(e)}")
        raise
